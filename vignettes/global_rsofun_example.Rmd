...
settings <- list(
  ...
  dir_out     = "~/data/grsofun_output/",     # path for tidy model output
  # dir_out_nc  = "/data/scratch/bstocker/grsofun_output/",     # xxx not yet handled
  dir_out_nc  = "~/data/grsofun_output/",     # xxx not yet handled
  ...
  dir_out_drivers = "~/data/grsofun_input/",  # If a path is provided rsofun driver object is to be saved. Uses additional disk space but substantially speeds up grsofun_run(). If dir_out_drivers is 'NA' driver object are not stored.
  ...
  dir_in_climate       = "~/data/watch_wfdei/", # path to where climate forcing data is located
  ...
  dir_out_tidy_climate = "~/data/watch_wfdei/tidy",  # path to where tidy climate forcing data is to be written
  ...
  file_in_fapar = "~/data/MODIS-C006_MOD15A2_LAI_FPAR_zmaw/MODIS-C006_MOD15A2__LAI_FPAR__LPDAAC__GLOBAL_0.5degree__UHAM-ICDC__2000_2018__MON__fv0.02.nc",  # path to where fAPAR forcing data is located
  dir_out_tidy_fapar = "~/data/MODIS-C006_MOD15A2_LAI_FPAR_zmaw/tidy/",  # path to where tidy fAPAR forcing data is to be written
  ...
  file_in_whc = "~/data/mct_data/cwdx80_forcing_halfdeg.nc",
  ...
  dir_out_tidy_whc = "~/data/mct_data/tidy/", # path to where tidy  root zone storage capacity forcing data is to be written

  # file_in_landmask      = "/data/archive/wfdei_weedon_2014/data/WFDEI-elevation.nc", # path to where land mask data is located
  file_in_landmask = "~/data/watch_wfdei/WFDEI-elevation.nc", # path to where land mask data is located
  ...
  dir_out_tidy_landmask = "~/data/watch_wfdei/tidy", # path to where tidy land mask data is to be written

  file_in_gicew = "~/tmp/gicew_halfdeg.cdf",
  dir_out_tidy_gicew = "~/tmp/gicew_tidy/",
  ...
  file_in_elv = "~/data/watch_wfdei/WFDEI-elevation.nc",        # path to where elevation data is located
  dir_out_tidy_elv = "~/data/watch_wfdei/tidy" # path to where tidy elevation data is to be written
)
```
...
settings$dir_out <- "~/data/grsofun_output/tmp1/"
tictoc::tic()
error <- suppressMessages(grsofun_run(par, settings))
tictoc::toc()

# not parallel
settings$ncores_max <- 1
settings$dir_out <- "~/data/grsofun_output/tmp2/"
tictoc::tic()
error <- suppressMessages(grsofun_run(par, settings))
tictoc::toc()
```
